...
{
    //std::string file_text = "Jason Gionfriddo\nThis is my test string";

    // TODO: implement loading the file into a string
    try
    {
        // declare the input files stream variable
        std::ifstream inputfile(filename);
        std::string file_text;
        // positions of the charaters to be read, start to end
        inputfile.seekg(0, std::ios::end);
        file_text.reserve(inputfile.tellg());
...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
    ...
    try {
        ...
        if (writeFile.is_open()) {
            ...
            // keep the timestamp to 12 characters
            char buffer[12];
            struct tm ltm;
            time_t now = time(0);
            localtime_s(&ltm, &now);
            std::strftime(buffer, 12, "%Y-%m-%d", &ltm);
            std::puts(buffer);
