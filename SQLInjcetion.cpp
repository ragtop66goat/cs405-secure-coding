...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    records.clear();
    // this will check for the suspicious pattern along with one or more matchong characters on both sides of the = sign 
    std::regex letters(" or '[a-zA-Z]+'='[a-zA-Z]+';");
    // this is the second test for suspicious pattern only with numbers
    std::regex numbers(" or [1-9]+=[1-9]+;");
    //convert the sql string passed to run_query to a null-terminated string; destroys pointer 
    std::string tempString = sql.c_str();
    std::string newString;
    // flag for matching the regex test
    std::smatch match;
    

   
    if (std::regex_search(tempString, match, letters)) {
        // alert for injection
        std::cout << "\nSQL Injection suspected!!!";
        // replace the injection with ; so program can finish
        newString = std::regex_replace(tempString, letters, ";");
        return run_query(db, newString, records);
    }

    
    else if (std::regex_search(tempString, match, numbers)) {
            // alert for injection
            std::cout << "\nSQL Injection suspected!!!";
            // replace the injection with ; so program can finish
            newString = std::regex_replace(tempString, numbers, ";");
            return run_query(db, newString, records);
    }
